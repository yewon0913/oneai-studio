...
## v3.25 image-analyzer.ts 최강 프롬프트 엔진 v2.0으로 교체 (invokeLLM 기반)
- [x] image-analyzer.ts를 invokeLLM 기반 최강 프롬프트 엔진으로 수정 (Anthropic API 크레딧 부족 해결)
...
- [x] invokeLLM 기반으로 교체 (내장 LLM 사용, 추가 비용 없음)
- [x] TypeScript 에러 0개 확인
- [x] 테스트 75개 통과 확인
- [x] 개인 사진 face consistency 1.5 검증 완료 (일관성 우수)
- [x] 커플 사진 문제는 face-swap 파이프라인에서 별도 해결 결정
## v3.26 규칙 기반 프롬프트 생성 시스템 (LLM 제거)
- [x] image-analyzer.ts를 규칙 기반 시스템으로 재구현 (invokeLLM 제거)
- [x] 이미지 분석 로직을 규칙 기반 CV 분석으로 변경
- [x] 분석 결과를 직접 프롬프트로 변환하는 매핑 함수 구현
