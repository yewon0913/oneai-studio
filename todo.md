...
## v3.53 개인촬영 탭 싱크로율 개선 - 배경/의류 정보 자동 추출 및 프롬프트 강화 (65% → 85-90%)
- [ ] 참조 이미지 분석 시 배경 색상/타입 자동 감지 (흰색, 회색, 자연배경 등)
- [ ] 참조 이미지 분석 시 의류 색상/스타일 자동 감지 (상의 색상, 소재 등)
- [ ] 개인촬영 탭 분석 결과에 배경/의류 정보 추가 표시
- [ ] 프롬프트 자동 적용 시 배경/의류 정보를 배경 프롬프트보다 우선 적용
- [ ] 네거티브 프롬프트에 "다른 배경, 다른 의류" 명시
- [ ] 싱크로율 테스트 (배경/의류 정보 포함 vs 미포함 비교)
- [ ] TypeScript 에러 0개 확인
- [ ] 테스트 75/75 통과 확인


## v3.54 개인촬영 탭 안경 착용 고정 체크박스 추가 - 안경 스타일 일관성 유지
- [ ] 개인촬영 탭 UI에 "안경 착용 고정" 체크박스 추가 (faceFixMode 아래)
- [ ] 상태 변수 추가: glassesFixMode (boolean)
- [ ] 프롬프트에 "keep glasses, same glasses style" 자동 추가 (glassesFixMode 활성화 시)
- [ ] 네거티브 프롬프트에 "no glasses removal, different glasses" 추가 (glassesFixMode 활성화 시)
- [ ] 생성 함수에서 glassesFixMode 파라미터 전달
- [ ] UI 테스트 (체크박스 동작 확인)
