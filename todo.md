...
## v3.5 캐릭터 시트 자동 생성 기능
- [x] DB 스키마에 고객 나이(age) 필드 추가
- [x] 고객 등록/편집 UI에 나이 입력 필드 추가
- [x] 추가 사진 영역 삭제
- [x] 추가 사진 자리에 캐릭터 시트 자동 생성 UI 구현
- [x] 캐릭터 시트 프롬프트 엔진 구현 (정면/측면 사진 기반, 나이 반영)
- [x] 캐릭터 시트 생성 서버 프로시저 구현 (originalImages로 직접 전달)
- [x] 캐릭터 시트 결과 미리보기 및 저장
- [x] 테스트 업데이트 및 검증 (77개 테스트 전체 통과)

## v3.5 버그 수정
- [x] 배포 사이트 '예상치 못한 오류' 런타임 에러 수정 (SSR-unsafe localStorage/document 접근 보호)
