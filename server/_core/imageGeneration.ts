/**
 * Image generation helper v5.0 — FLUX.2 Primary + PuLID + Gemini Fallback
 *
 * 모델 우선순위:
 *   1. FLUX.2 LoRA (fal-ai/flux-2/lora) — Primary
 *   2. PuLID (fal-ai/flux-pulid) — 얼굴 일관성 후처리 (참조 이미지 있을 때)
...
export async function generateImage(
    ...
    if (refImageBase64) {
      try {
        ...
        if (faceRefUrl) {
          ...
          console.log('[PuLID] 응답 키:', Object.keys(pulidResult || {}));
          const pulidUrl = (pulidResult?.images as Array<{ url: string }>)?.[0]?.url;
          if (pulidUrl) {
            finalUrl = pulidUrl;
            console.log('[PuLID] 성공 ✅:', pulidUrl.slice(0, 60));
          } else {
            console.log('[PuLID] 실패 - 원본 유지');
          }
        }
      } catch (pulidErr: any) {
        console.warn(`[PuLID] 에러 (원본 유지): ${pulidErr.message?.slice(0, 100)}`);
      }
    }
  ...
  throw new Error("모든 이미지 생성 모델 실패 (FLUX.2+PuLID → Gemini)");
}
